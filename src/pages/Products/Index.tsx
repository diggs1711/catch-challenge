...
const ProductsPage = () => {
  ...
  const [selectOption, setSelectedOption] = useState<ProductPageSortOptions>(
    "High"
  );
  ...
  const sortProducts = (a: ProductDetails, b: ProductDetails) => {
    return selectOption === "High"
      ? b.salePrice - a.salePrice
      : a.salePrice - b.salePrice;
  };

  const createProductCard = (product: ProductDetails) => (
    <ProductCard key={product.id} details={product} />
  );

  const productCards = productData.results
    .sort(sortProducts)
